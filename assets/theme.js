...
document.addEventListener('DOMContentLoaded', function() {
  ...
  sections.register('collection-template', theme.coll_product);
...
// Pagination on Load More Click Collection.
var has_pagination = false;
document.addEventListener('click',function(e){ 
  var parser = new DOMParser();     
  if(e.target && e.target.id == 'load_pagiantion'){
    e.preventDefault();                
    var next_url = e.path[0].href;    
    var fetch_html = parser.parseFromString(e.target.outerHTML, 'text/html');
    var fetch_url = fetch_html.body.querySelector('#load_pagiantion').getAttribute('href')    
    if(fetch_url != '' ){  
      e.path[0].remove();
      const xhttp = new XMLHttpRequest();  
      xhttp.onload = function() {      
        var next_html =xhttp.responseText;
        var doc = parser.parseFromString(next_html, 'text/html');            
        var get_html = doc.body.querySelector('#Collection');
        var btn_pag = doc.body.querySelector('.btn--narrow')
        document.getElementById('Collection').appendChild(get_html);                                                            
      }
      xhttp.open("GET", fetch_url);
      xhttp.send();
    }else{
      alert('Product Loaded');      
      document.querySelector('#load_pagiantion')[0].innerHTML = "Scroll Top";
      console.log(fetch_html.body.querySelector('#load_pagiantion').innerHTML,'fetch_htmlfetch_html');
    }
  }
});          
